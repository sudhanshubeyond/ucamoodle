...
function applyRadiusPreview() {
    ...
    if (btnRadius !== '' && parseInt(btnRadius) > 0) {
...
function applyIconSet(iconset) {
    var fontFamily = iconset === 'iconset-set1' ? 'Remui-v1' : 'Remui';
...
function apply() {
    ...
    $(Utils.getDocument()).find('body').toggleClass('modern-preset', isModern);
...
function init() {
    ...
    if ($initiallyChecked.length) {
        ...
        $(Utils.getDocument()).find('body').toggleClass('modern-preset', initiallyModern);
    }
     ...
     $(SELECTOR.PRESETAPPLY).on('click', function() {
        ...
        setTimeout(function() {
            ...
            if ($checked.val() === 'preset-modern') {
                ...
                // Header design settings — element background color → transparent.
                $('[name="header-menu-element-bg-color"]').spectrum('set', 'rgba(0, 0, 0, 0)').trigger('color.changed');
            ...
            // Update footer widget logo to reflect the active preset.
            footer.useFooterWidgetLogo(true, true);
